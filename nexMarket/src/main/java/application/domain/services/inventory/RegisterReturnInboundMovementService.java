...
public class RegisterReturnInboundMovementService {

    private final InventoryRepositoryPort inventoryRepositoryPort;
    private final InventoryMovementRepositoryPort inventoryMovementRepositoryPort;

    public RegisterReturnInboundMovementService(InventoryRepositoryPort inventoryRepositoryPort,
    ...
    public InventoryMovement registerReturnInbound(Inventory inventory, int quantity, Order order, User performedBy) {
        if (quantity <= 0) {
            throw new DomainException("La cantidad reingresada por devolución debe ser mayor a cero.");
        }

        Inventory existingInventory = inventoryRepositoryPort.findByProductAndWarehouse(inventory)
                .orElseThrow(() -> new EntityNotFoundException("Inventario no encontrado para registrar reingreso por devolución."));
        ...
        movement.setMovementType(InventoryMovementType.RETURN_INBOUND);
